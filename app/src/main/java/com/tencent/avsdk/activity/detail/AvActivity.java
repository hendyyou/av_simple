package com.tencent.avsdk.activity.detail;
...
import com.tencent.avsdk.activity.GameOverActivity;
import com.tencent.avsdk.activity.base.BaseActivity;
import com.tencent.avsdk.model.ChatEntity;
import com.tencent.avsdk.adapter.ChatMsgListAdapter;
import com.tencent.avsdk.view.CircularImageButton;
import com.tencent.avsdk.constants.DemoConstants;
import com.tencent.avsdk.utils.HttpUtil;
import com.tencent.avsdk.utils.ImageUtil;
...
import com.tencent.avsdk.dialog.MemberListDialog;
 ...
 * 直播界面
 */
public class AvActivity extends BaseActivity implements OnClickListener {
    ...
    private ImageButton btn_switchCamera;//前置后置摄像头的转换
    private CircularImageButton hostHead;
    private ImageButton btn_mic;//语音按钮
    ...
    protected void findViewByIDS() {
        mainView = (RelativeLayout) findViewById(R.id.av_screen_layout);
        mClockTextView = (TextView) findViewById(R.id.qav_timer);//直播时间
        btn_switchCamera = (ImageButton) findViewById(R.id.activity_av_imabtn_switchcamera);//转换摄像头
        hostHead = (CircularImageButton) findViewById(R.id.activity_av_imabtn_host_head);//主播头像
        btn_mic = (ImageButton) findViewById(R.id.activity_av_imabtn_mic_btn);//语音按钮

        mBottomBar = (FrameLayout) findViewById(R.id.activity_av_include_qav_bottom_bar);//底部include
        mListViewMsgItems = (ListView) findViewById(R.id.activity_av_im_msg_items);//消息列表
        mEditTextInputMsg = (EditText) findViewById(R.id.activity_av_qav_bottombar_msg_input);//底部编辑框
        mEditTextInputMsg.setOnClickListener(this);
        mButtonSendMsg = (Button) findViewById(R.id.activity_av_btn_qav_bottombar_send_msg);//发送
        mButtonSendMsg.setOnClickListener(this);

        mButtonBeauty = (TextView) findViewById(R.id.activity_av_tv_beauty_btn);//美颜按钮
        mButtonBeauty.setOnClickListener(this);
        mBeautySettings = (LinearLayout) findViewById(R.id.activity_av_include_qav_beauty_setting);//美颜include
        mBeautyBar = (SeekBar) (findViewById(R.id.activity_av_seekbar_qav_beauty_progress));//美颜滑动条
        mBeautyConfirm = (TextView) findViewById(R.id.activity_av_tv_qav_beauty_setting_finish);//美颜结束
        ...
        findViewById(R.id.qav_topbar_hangup).setOnClickListener(this);//关闭直播
        findViewById(R.id.qav_topbar_push).setOnClickListener(this);//旁路直播
        findViewById(R.id.qav_topbar_record).setOnClickListener(this);//录制
        findViewById(R.id.qav_topbar_streamtype).setOnClickListener(this);//HLS

        praiseLayout = (LinearLayout) findViewById(R.id.praise_layout);//爱心的layout
        mPraiseNum = (TextView) findViewById(R.id.text_view_live_praise);//爱心数量
        mButtonPraise = (ImageButton) findViewById(R.id.image_btn_praise);//爱心图片
        mButtonPraise.setOnClickListener(this);

        mMemberListButton = (TextView) findViewById(R.id.btn_member_list);//观众列表按钮
    ...
    private boolean mIsSuccess = false;//是否成功开启surface

    private QavsdkControl mQavsdkControl;//直播SDK
    private UserInfo mSelfUserInfo;//用户信息
    private ArrayList<MemberInfo> mMemberList, mVideoMemberList, mNormalMemberList;
    ...
    private PowerManager.WakeLock wakeLock;//熄屏
    ...
    private List<ChatEntity> mArrayListChatEntity;//聊天的集合
    private ChatMsgListAdapter mChatMsgListAdapter;
    private boolean mIsLoading = false;//是否正在加载中
    ...
    private String groupId;//讨论组组ID
    private int praiseNum;//爱心数

    public static Intent newInstance(Context context, int roomNum, String mHostIdentifier, String groupId, int praiseNum) {
        Intent in = new Intent(context, AvActivity.class);
        Bundle bundle = new Bundle();
        in.putExtras(bundle);
        in.putExtra(Util.EXTRA_ROOM_NUM, roomNum); //room id
        in.putExtra(Util.EXTRA_SELF_IDENTIFIER, mHostIdentifier);
        in.putExtra(Util.EXTRA_GROUP_ID, groupId);// chat converse id
        in.putExtra(Util.EXTRA_PRAISE_NUM, praiseNum);
        return in;
    }
    ...
    private void initFirst() {
        showDialog(DIALOG_INIT);//Dialog的创建
        Util.switchWaitingDialog(mContext, mDialogInit, DIALOG_INIT, true);//Dialog的显示

        registerBroadcastReceiver();//注册2个接收者
        mQavsdkControl = mQavsdkApplication.getQavsdkControl();//直播sdk的获取
        ...
        if (netType != AVConstants.NETTYPE_NONE) {
            mQavsdkControl.setNetType(Util.getNetWorkType(mContext));
        }

        //在使用SDK前，需要创建一个AVContext(SDK上下文)对象。
        if (mQavsdkControl.getAVContext() != null) {
            //todo 直播UI，麦克风，摄像头 初始化 ---step00
            mQavsdkControl.onCreate((QavsdkApplication) getApplication(), findViewById(android.R.id.content));//andorid.R.id.content即为跟视图
        } else {
            finish();
        }
...
//			groupForPush = Integer.parseInt(mSelfUserInfo.getUserPhone().substring(0, 5));
        ...
        groupId = getIntent().getExtras().getString(Util.EXTRA_GROUP_ID);//小组的ID
        //是否为直播者
    ...
    private void initView() {
        ...
        if (mSelfUserInfo.isCreater()) {
            ...
            avAudioCtrl.enableMic(true);//麦克风可用
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Util.ACTION_SURFACE_CREATED)) {
                //todo 【收】SufrfaceView创建成功 ---step01
                locateCameraPreview();
                wakeLock.acquire();
...
//                    }
                ...
                if (!TextUtils.isEmpty(mRecvIdentifier)) {
                    mQavsdkControl.setRemoteHasVideo(false, mRecvIdentifier, 0);//关掉远程的video
                }

                mRecvIdentifier = identifier;
            } else if (action.equals(Util.ACTION_VIDEO_SHOW)) {
                //TODO 【收3】VIDEO 显示 ---step03 ---游客
                //成员模式加入视频聊天室
                String identifier = intent.getStringExtra(Util.EXTRA_IDENTIFIER);
                ...
                //发消息通知大家 自己上线了
                onMemberEnter();
                Util.switchWaitingDialog(mContext, mDialogInit, DIALOG_INIT, false);
            } else if (action.equals(Util.ACTION_ENABLE_CAMERA_COMPLETE)) {
                //TODO 【收】摄像头初始化完成  ---step0201
...
//                    break;
    ...
    private void closeActivity() {
        ...
        if (mSelfUserInfo.isCreater() == true) {
            ...
            Util.switchWaitingDialog(mContext, mDialogAtDestroy, DIALOG_DESTROY, true);
...
//        getMessage();
        ...
        TIMManager.getInstance().addMessageListener(msgListener);//聊天的监听者
...
//                TIMManager.getInstance().logout();
                Toast.makeText(mContext, "加群失败,失败原因：" + i + ":" + s, Toast.LENGTH_SHORT).show();
            }
...
//    };
    ...
    public void hostRequestView(String identifier) {
        ...
        if (endpoint != null) {
            ...
            //成员模式请求界面
            //todo 【发3】VIDEO 显示 ---游客
            mContext.sendBroadcast(new Intent(Util.ACTION_VIDEO_SHOW)
...
//            endpoint.hostRequestView(view, mRequestViewCompleteCallback);
            //成员模式请求界面
            mContext.sendBroadcast(new Intent(Util.ACTION_MEMBER_VIDEO_SHOW)
                    .putExtra(Util.EXTRA_IDENTIFIER, identifier)
                    .putExtra(Util.EXTRA_VIDEO_SRC_TYPE, view.videoSrcType));
        } else {
            mEditTextInputMsg.setVisibility(View.GONE);
...
//                    onCloseVideo();
                ...
                break;
            case R.id.activity_av_tv_beauty_btn://美颜
            ...
            case R.id.activity_av_tv_qav_beauty_setting_finish:
                mBeautySettings.setVisibility(View.GONE);
                mBottomBar.setVisibility(View.VISIBLE);
                mMemberListButton.setVisibility(View.VISIBLE);
                break;
            case R.id.activity_av_imabtn_switchcamera:
                onSwitchCamera();
                break;
            case R.id.activity_av_btn_qav_bottombar_send_msg://底部发送信息
                onSendMsg();
                break;
            case R.id.activity_av_qav_bottombar_msg_input:
                mIsClicked = true;
                break;
...
//                uploadRecordToServer();
    ...
    protected void onCheckedChanged(boolean checked) {
        ...
        if (checked) {
            ...
            btn_mic.setBackgroundResource(R.drawable.mic1);
            avAudioCtrl.enableMic(true);
        } else {
            Log.d(TAG, "audio Mic set false ");
            avAudioCtrl.enableMic(false);
            btn_mic.setBackgroundResource(R.drawable.mic3);
        }
    }
...
//            }
        AVAudioCtrl avAudioCtrl = mQavsdkControl.getAVContext().getAudioCtrl();
        avAudioCtrl.enableMic(true);//打开麦克风
        OpenVoice = true;
    }
...
//        mQavsdkControl.setRequestCount(0);
    ...
    protected void onDestroy() {
        ...
        Log.d(TAG, "WL_DEBUG onDestroy");
        Util.switchWaitingDialog(mContext, mDialogAtDestroy, DIALOG_DESTROY, false);
    }
}