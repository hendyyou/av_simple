package com.tencent.avsdk.activity.login_register;
...
import com.tencent.avsdk.activity.main.StartActivity;
...
public class LoginActivity extends Activity implements TextWatcher, View.OnClickListener {
    ...
    private int accType = Integer.parseInt(DemoConstants.ACCOUNTTYPE);//账号类型
    private int sdkAppid = DemoConstants.APPID;//sdkID值
    ...
    public void getUserInfo(final String Usersig) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                String response = HttpUtil.PostUrl(HttpUtil.UserInfoUrl, params);//网络交互
                ...
                try {
                    ...
                    if (ret == 200) {
                        ...
                        setUserInfo(sigobj, userphone, Usersig);//个人信息的解析并存储在本地
                        //todo 腾讯IM的初始化
                        TIMManager.getInstance().setEnv(mSelfUserInfo.getEnv());
